...
const days          = getDaysSinceCycleStart(today)
...
const estacaoAtual  = getCurrentSeason(new Date());
const moon          = getMoonEmoji(getRealLunarDay(days))
...
export const dados = {
  ...
  export function getLunisolarDate(date: Date) {
...
export function getDaysSinceCycleStart(date: Date) {
  const millisecondsPerDay = 1000 * 60 * 60 * 24

  const difference =
    date.getTime() - CYCLE_START.getTime()

  return Math.floor(difference / millisecondsPerDay)
}

export function getPoeticMonth(daysSinceStart: number) {
  const monthIndex = Math.floor(daysSinceStart / SYNODIC_MONTH) % 235

  return poeticMonths[monthIndex]
}

export function getLunarDay(daysSinceStart: number) {
  return (daysSinceStart % 30) + 1
}

export function getPrimavera(daysSinceStart: number) {
  const yearsSinceStart = Math.floor(daysSinceStart / SYNODIC_MONTH) % 235

  return (getPrimaveraLuni(yearsSinceStart))
}

export function getCycle(daysSinceStart: number) {
  const yearsSinceStart = Math.floor(daysSinceStart / 360)

  return Math.floor(yearsSinceStart / 19)
}

export function getRealLunarAge(daysSinceStart: number) {
  return daysSinceStart % SYNODIC_MONTH
}

export function getRealLunarDay(daysSinceStart: number) {
  const lunarAge =
    getRealLunarAge(daysSinceStart)

  return Math.floor(lunarAge) + 1
}

export function getPrimaveraLuni(monthNumber: number): number {
...
export function getReflection() { return `O horizonte escurece lentamente, mas nada do que cumpriu sua função se perde.`}
